...
export default function Calculateur() {
  var ref = useLang(), lang = ref.lang;
  var _buyPrice = useState(""), buyPrice = _buyPrice[0], setBuyPrice = _buyPrice[1];
  var _costs = useState(""), costs = _costs[0], setCosts = _costs[1];
  var _targetMargin = useState("30"), targetMargin = _targetMargin[0], setTargetMargin = _targetMargin[1];
  ...
  function calculate() {
    ...
    var extra = parseFloat(costs) || 0;
    var margin = parseFloat(targetMargin) / 100 || 0.30;
    var totalCost = buy + extra;

    var res = Object.entries(PLATFORM_FEES).map(function(entry) {
      var name = entry[0], fees = entry[1];
      var breakEven;
      if (fees.commission > 0) { breakEven = (totalCost + fees.fixed) / (1 - fees.commission); }
      else { breakEven = totalCost + fees.fixed; }

      var recommended;
      if (fees.commission > 0) { recommended = (totalCost * (1 + margin) + fees.fixed) / (1 - fees.commission); }
      else { recommended = totalCost * (1 + margin) + fees.fixed; }

      recommended = Math.ceil(recommended * 2) / 2;
      breakEven = Math.ceil(breakEven * 2) / 2;

      var platformFee = recommended * fees.commission + fees.fixed;
      var netProfit = recommended - totalCost - platformFee;
      var actualMargin = totalCost > 0 ? (netProfit / totalCost) * 100 : 0;

      return { name: name, breakEven: breakEven.toFixed(2), recommended: recommended.toFixed(2), platformFee: platformFee.toFixed(2), netProfit: netProfit.toFixed(2), actualMargin: actualMargin.toFixed(0), color: fees.color, bg: fees.bg, commissionPct: Math.round(fees.commission * 100) };
    });
